...
from utils import init_weights, kl_criterion,  finn_eval_seq, mse_criterion, pred, plot_pred
...
def parse_args():
    ...
    parser.add_argument('--batch_size', default=4,
                        type=int, help='batch size') #default 12; good 4
    parser.add_argument('--log_dir', default='./logs/fp',
    ...
    parser.add_argument('--niter', type=int, default=150,
                        help='number of epochs to train for')  # 150
    parser.add_argument('--epoch_size', type=int,
                        default=600, help='epoch size')
    parser.add_argument('--tfr', type=float, default=1.0, #good: 1.0, 0.0  #bad:0.5
                        help='teacher forcing ratio (0 ~ 1)')
    parser.add_argument('--tfr_start_decay_epoch', type=int, default=75, #0
                        help='The epoch that teacher forcing ratio become decreasing')
    parser.add_argument('--tfr_decay_step', type=float, default=1/150, 
    ...
    parser.add_argument('--kl_anneal_cyclical', default=False,
                        action='store_true', help='use cyclical mode')
    parser.add_argument('--kl_anneal_ratio', type=float,
                        default=0.5, help='The decay ratio of kl annealing')
    ...
    parser.add_argument('--last_frame_skip', action='store_true',default=False,
...
def train(x, cond, modules, optimizer, kl_anneal, args, device):
    ...
    # (batch_size*30*3*64*64) -> (30*batch_size*3*64*64)
    x = x.permute(1, 0, 2, 3, 4)
    cond = cond.permute(1, 0, 2)  # (batch_size*30*7) -> (30*batch_size*7)
    x = x.to(device)
    cond = cond.to(device)

    # lots of tuple (h5, [h1, h2, h3, h4])
    h_seq_gt = [modules['encoder'](x[i])
                for i in range(args.n_past + args.n_future)]
    x_pred_seq = []
    x_pred_seq.append(x[0])
    for i in range(1, args.n_past + args.n_future):
        h_target = h_seq_gt[i][0]
        if args.last_frame_skip or i < args.n_past:
            if(use_teacher_forcing):
                h, skip = h_seq_gt[i-1]
            else:
                h, skip = modules['encoder'](
                    x_pred_seq[i-1])  # skip代表skip connection
        else:
            if(use_teacher_forcing):
                h, _ = h_seq_gt[i-1]
            else:
...
class kl_annealing():
    def __init__(self, args):
        ...
        total_iter = args.niter * args.epoch_size
        self.beta_list = np.ones(total_iter)
        ...
        self.current_iter = 0
        period = total_iter/cycle
        self.ratio = args.kl_anneal_ratio
        if self.ratio != 0:
            step = 1.0/(period * self.ratio)

            for i in range(cycle):
                j, k = 0, 0
                while j <= 1 and (int(k + j*period) < total_iter):
                    self.beta_list[int(k + j*period)] = j
                    j += step
                    k += 1
    ...
    def get_beta(self):
        if self.ratio != 0:
            beta = self.beta_list[self.current_iter]
        else:
            beta = 0
        self.update()
        return(beta)
...
def main():
    ...
    # make sure some parameters are in the right range
    ...
    if args.model_dir != '':
        ...
        # update some new information in args
    ...
    # insure the random number always the same
    ...
    if args.model_dir != '':
        ...
        frame_predictor = lstm(args.g_dim+args.z_dim+7, args.g_dim,
        ...
        # apply 方法會遞迴地對模型的所有子模型進行初始化，並將指定的函數應用到每個子模型的權重上。
        # 在這個程式碼中，指定的函數是init_weights，它會對模型的權重進行初始化
    ...
    # num_workers 是指定使用多少個 worker 來載入資料
    # num_workers 的值通常設定為 CPU 的核心數量，或是稍微小於核心數量的值，以避免過度佔用 CPU 資源。
    train_loader = DataLoader(train_data,
                              ...
                              drop_last=True,
                              pin_memory=True)
    # 透過iter()函數，可以將train_loader轉換成一個迭代器物件，並將其賦值給train_iterator。
    # 這樣做的好處是可以透過for迴圈來遍歷(train_iterator)中的每一個元素，也就是每一個訓練樣本，以便進行模型訓練。
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        for _ in range(args.epoch_size):
            ...
            loss, mse, kld = train(
                seq, cond, modules, optimizer, kl_anneal, args, device)
            epoch_loss += loss
            epoch_mse += mse
            epoch_kld += kld
        ...
        if epoch % 5 == 0:
            ...
            for _ in range(len(validate_data) // args.batch_size):
                try:
                    # validate_seq = (batch_size*30*3*64*64); validate_cond = (batch_size*30*7)
                    validate_seq, validate_cond = next(validate_iterator)
                except StopIteration:
                    validate_iterator = iter(validate_loader)
                    validate_seq, validate_cond = next(validate_iterator)

                pred_seq = pred(validate_seq, validate_cond,
                                modules, args, device)
                validate_seq = validate_seq.permute(1, 0, 2, 3, 4)
                _, _, psnr = finn_eval_seq(
                    validate_seq[args.n_past:args.n_past + args.n_future], pred_seq[args.n_past:])
