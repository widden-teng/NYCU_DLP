...
class ReplayMemory:
    ...
    def sample(self, batch_size, device):
        ...
        return (torch.tensor(x, dtype=torch.float, device=device)
                for x in zip(*transitions))  # 不確定type對不對
...
class DDPG:
    ...
    # Python中的靜態方法，它可以在不需要實例化類別的情況下直接調用。靜態方法通常用於定義一些與類別相關的
    # 功能，但不需要訪問類別的屬性或方法。
    @staticmethod
    def _update_target_network(target_net, net, tau):
        ...
        for target, behavior in zip(target_net.parameters(), net.parameters()):
            ## TODO ##
            # .data 是一個 Tensor 屬性，用於獲取 Tensor 的數值資料，而不包含任何關於梯度的資訊。
            # 這個屬性可以用來獲取 Tensor 的數值資料，並且可以直接修改 Tensor 的數值資料，
            # 而不會影響到梯度的計算。
            ### .copy_() 是 PyTorch 中 Tensor 的一個方法，用於將一個 Tensor 的數值資料複製到另一個 Tensor 中
            # .copy_()方法被用來將加權平均後的行為網路的權重複製到目標網路的權重中，以便進行更新
